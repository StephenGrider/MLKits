...
class LogisticRegression {
  ...
  train() {
    ...
    for (let i = 0; i < this.options.iterations; i++) {
      for (let j = 0; j < batchQuantity; j++) {
        ...
        /**
         * tf.tidy() ensures that we only retain the tensors in memory that we
         * return.
         */
  ...
  standardize(features) {
    ...
    // prevent variance division by zero below when standarizing
    // this changes zeros to 1s.
    ...
    this.variance = this.variance || variance.add(filler);
